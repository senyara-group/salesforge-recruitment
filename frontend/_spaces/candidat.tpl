...
<title>Espace candidat - SalesForge</title>
...
.nd{position:absolute;top:6px;right:6px;width:7px;height:7px;background:var(--r);border-radius:50%;border:2px solid var(--w)}
...
.nav-dot{position:absolute;top:3px;right:26%;width:8px;height:8px;background:var(--r);border-radius:50%;border:2px solid var(--w)}
...
.doc-item{background:var(--w);border:1px solid var(--bd);border-radius:12px;padding:12px 13px;display:flex;align-items:center;gap:11px;text-decoration:none;color:inherit;transition:.15s}
.doc-item:hover{border-color:var(--bm);background:var(--bs)}
.doc-num{width:34px;height:34px;border-radius:10px;background:var(--bs);color:var(--b);display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-weight:900;flex-shrink:0}
.doc-info{flex:1;min-width:0}
.doc-title{font-family:'Sora',sans-serif;font-size:13px;font-weight:800;line-height:1.3}
.doc-meta{font-size:11px;color:var(--ink3);margin-top:2px}
...
.filter-row{display:flex;gap:6px;flex-wrap:wrap;margin-top:9px}
...
.swipe-card{position:absolute;inset:0;background:var(--w);border:1px solid var(--bd);border-radius:22px;overflow:hidden;cursor:grab;touch-action:none;user-select:none;display:flex;flex-direction:column;box-shadow:none;will-change:transform;z-index:0}
...
.card-match-box{position:absolute;top:12px;right:12px;background:var(--w);border-radius:10px;padding:6px 10px;text-align:center;box-shadow:0 3px 10px rgba(0,0,0,.13)}
...
.card-co{position:absolute;bottom:12px;left:15px;color:#fff}
...
.card-body{flex:1;padding:13px 17px;overflow-y:auto;display:flex;flex-direction:column;gap:10px}
...
.deck-actions{position:sticky;bottom:70px;background:var(--w);border-top:1px solid var(--bd);padding:12px 18px;display:flex;align-items:center;justify-content:center;gap:11px}
...
.db-like{width:58px;height:58px;background:var(--b);border-color:var(--b);color:#fff;box-shadow:none}.db-like:hover{background:var(--b2)}
...
.analyzing{padding:50px 18px;text-align:center}
...
.res-card{background:var(--ink);color:#fff;border-radius:20px;padding:24px 20px;text-align:center;margin-bottom:13px}
.res-tag{display:inline-block;font-size:10px;font-weight:700;color:rgba(155,185,255,1);background:rgba(19,64,224,.2);padding:4px 11px;border-radius:6px;text-transform:uppercase;margin-bottom:12px}
.res-s{font-family:'Sora',sans-serif;font-size:62px;font-weight:900;letter-spacing:-3px;line-height:1;margin-bottom:3px}
.res-d{font-size:12px;color:rgba(255,255,255,.42);margin-bottom:9px}
.res-t{font-family:'Sora',sans-serif;font-size:17px;font-weight:800;margin-bottom:7px}
.res-desc{font-size:13px;line-height:1.7;color:rgba(255,255,255,.62);max-width:300px;margin:0 auto 14px}
.res-axes{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-bottom:14px}
.res-ax{background:rgba(255,255,255,.07);border-radius:10px;padding:11px 7px}
.res-ax-v{font-family:'Sora',sans-serif;font-size:20px;font-weight:900}
.res-ax-l{font-size:10px;color:rgba(255,255,255,.42);margin-top:3px;font-weight:600}
.res-tags{display:flex;gap:5px;justify-content:center;flex-wrap:wrap;margin-bottom:16px}
.res-tp{background:rgba(255,255,255,.1);color:#fff;padding:5px 11px;border-radius:13px;font-size:11px;font-weight:600}
.res-cta{display:flex;flex-direction:column;gap:7px}
.res-docs{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);border-radius:14px;padding:14px;margin:16px 0;text-align:left}
.res-docs-t{font-family:'Sora',sans-serif;font-size:14px;font-weight:900;color:#fff;margin-bottom:4px}
.res-docs-s{font-size:12px;color:rgba(255,255,255,.65);line-height:1.6;margin-bottom:12px}
.res-doc-actions{display:grid;grid-template-columns:1fr;gap:8px}
.res-doc-btn{width:100%;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.1);color:#fff;padding:11px 12px;border-radius:10px;font-family:'Sora',sans-serif;font-weight:800;cursor:pointer}
.res-doc-btn:hover{background:rgba(255,255,255,.16)}
.res-doc-status{display:none;margin-top:10px;font-size:12px;color:#8DFFC4;font-weight:700;text-align:center}
...
.ur-dot{width:8px;height:8px;border-radius:50%;background:var(--b);flex-shrink:0;margin-top:5px}
...
/* PROFIL */
.ph{background:var(--ink);color:#fff;padding:20px 18px}
.ph-top{display:flex;align-items:center;gap:12px;margin-bottom:15px}
.ph-main{flex:1;min-width:0}
.profile-edit-btn{width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.08);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
.profile-edit-btn svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2}
.ph-av{width:60px;height:60px;border-radius:50%;background:var(--b);display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-size:20px;font-weight:900;flex-shrink:0;overflow:hidden;color:#fff}
.ph-av img{width:100%;height:100%;object-fit:cover;display:block}
.ph-name{font-family:'Sora',sans-serif;font-size:17px;font-weight:900}
.ph-title{font-size:13px;color:rgba(255,255,255,.58);margin-top:2px}
.ph-rank{display:inline-flex;align-items:center;background:rgba(19,64,224,.25);color:#B0C8FF;padding:3px 10px;border-radius:6px;font-size:10px;font-weight:700;text-transform:uppercase;margin-top:5px}
.ph-badges{display:flex;gap:7px;flex-wrap:wrap;margin-top:9px}
.ph-badge{display:none;align-items:center;gap:5px;background:rgba(255,255,255,.09);color:#D7E4FF;border:1px solid rgba(255,255,255,.12);border-radius:7px;padding:4px 8px;font-size:10px;font-weight:700;text-transform:uppercase}
...
.ph-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:11px}
.profile-edit-panel{display:none;background:#101A28;border:1px solid rgba(176,196,216,.24);border-radius:14px;padding:18px;margin:18px 0;text-align:left;box-shadow:inset 0 1px 0 rgba(255,255,255,.04),0 16px 34px rgba(0,0,0,.16)}
.profile-edit-panel.on{display:block}
.profile-edit-head{display:flex;align-items:center;gap:12px;margin-bottom:16px}
.profile-edit-photo{width:58px;height:58px;border-radius:50%;background:var(--b);color:#fff;display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-size:18px;font-weight:900;overflow:hidden;flex-shrink:0}
.profile-edit-photo img{width:100%;height:100%;object-fit:cover;display:block}
.profile-edit-title{font-family:'Sora',sans-serif;font-size:14px;font-weight:900;color:#fff}
.profile-edit-sub{font-size:12px;color:rgba(255,255,255,.58);line-height:1.45;margin-top:2px}
...
.profile-edit-field label{font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:rgba(255,255,255,.5);font-weight:800}
.profile-edit-field input{width:100%;min-height:42px;border:1px solid rgba(176,196,216,.26);background:#1B2735;color:#fff;font-size:14px;font-weight:700;outline:none;padding:10px 12px;border-radius:9px}
.profile-edit-field input::placeholder{color:rgba(255,255,255,.45)}
.profile-edit-field input:focus{border-color:rgba(255,255,255,.48);background:#2D3948;box-shadow:0 0 0 3px rgba(255,255,255,.06)}
.profile-resource-list{display:grid;gap:8px;margin:12px 0}
.profile-resource-row{background:#1B2735;border:1px solid rgba(176,196,216,.22);border-radius:11px;padding:11px;display:grid;gap:9px}
.profile-resource-name{font-size:12px;font-weight:800;color:#fff;line-height:1.35;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.profile-resource-meta{font-size:11px;color:rgba(255,255,255,.54);margin-top:1px}
.profile-resource-actions{display:flex;gap:7px;flex-wrap:wrap}
.profile-resource-actions button,.profile-edit-actions .btn,.profile-edit-head .upload-btn{border:1px solid rgba(176,196,216,.28);background:#263240;color:#fff;padding:9px 11px;font-size:12px;font-weight:900;border-radius:9px;cursor:pointer;font-family:'Sora',sans-serif}
.profile-resource-actions button:hover,.profile-edit-actions .upload-btn:hover{background:#34404E;border-color:rgba(255,255,255,.32)}
.profile-resource-actions button:disabled{opacity:.42;cursor:not-allowed}
.profile-resource-actions .danger{color:#FFD5D5;border-color:rgba(204,47,47,.35);background:rgba(204,47,47,.12)}
.profile-edit-actions{display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:12px}
.profile-edit-actions .save-btn{background:var(--b);border-color:var(--b);box-shadow:0 10px 22px rgba(19,64,224,.28)}
.phs-v{font-family:'Sora',sans-serif;font-size:21px;font-weight:900}
.phs-l{font-size:10px;color:rgba(255,255,255,.48);text-transform:uppercase;margin-top:2px;font-weight:600}
...
@media (min-width:720px){
  ...
  #p-profile .ph{border-radius:18px;padding:28px;position:sticky;top:83px;align-self:start}
  ...
  #p-swipe .deck-area{grid-column:2;width:100%;height:min(660px,calc(100dvh - 220px));margin:18px 0 0}
...
@media (min-width:720px){
  ...
  #p-profile .ph{position:static;padding:38px 42px}
  ...
  .profile-edit-panel{max-width:none;padding:28px 24px;margin-top:22px;margin-bottom:22px}
  .profile-edit-grid{gap:28px;margin-bottom:28px}
  .profile-edit-field input{min-height:48px;font-size:17px}
  .profile-edit-actions .btn{width:min(420px,100%);min-height:48px;font-size:16px}
  #p-swipe.page.on{display:block;padding:0 0 96px}
  #p-swipe .swipe-hd{margin:0;padding:18px 42px}
  #p-swipe .deck-area{width:min(620px,calc(100% - 84px));height:min(720px,calc(100dvh - 230px));margin:22px auto 0}
...
@media (min-width:1180px){
  ...
  #p-home .home-hero,#p-profile .ph,#p-swipe .swipe-hd,.pricing-hd{padding-left:64px;padding-right:64px}
  #p-swipe .deck-area,#p-swipe .deck-actions{width:min(680px,calc(100% - 128px))}
  .plans-row{padding-left:64px;padding-right:64px}
}

@media (min-width:1180px){
  .profile-edit-panel{padding:26px 24px}
  .profile-edit-actions .btn{width:min(460px,100%)}
}
...
@media (min-width:1024px){
  ...
  #p-swipe .swipe-hd{
    top:52px;
    margin:0;
    padding:18px clamp(42px,4vw,64px);
  }
  #p-swipe .deck-area{
  ...
  #p-swipe .swipe-card{border-radius:22px 22px 0 0}
  #p-swipe .card-top{height:160px}
  #p-swipe .card-title{font-size:24px}
  #p-swipe .card-body{padding:18px 22px;gap:13px}
...
<div class="topbar" id="topbar" style="display:none">
  <div class="logo" onclick="go('home')"><div class="logo-m">S</div><div class="logo-n sf">Sales<b>Forge</b></div></div>
  <div class="top-r">
    <div class="ib" onclick="go('messages')"><svg viewBox="0 0 24 24"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg><div class="nd" id="msg-dot" style="display:none"></div></div>
...
<div id="p-home" class="page">
  <div class="home-hero">
    ...
    <div class="kpi-grid">
      <div class="kpi"><div class="kpi-l">Matchs</div><div class="kpi-v sf" id="kpi-m">—</div><div class="kpi-t tf" id="kpi-mt">—</div></div>
      <div class="kpi"><div class="kpi-l">Candidatures</div><div class="kpi-v sf" id="kpi-c">—</div><div class="kpi-t tf" id="kpi-ct">—</div></div>
...
<div id="p-coaching" class="page">
  <div class="sec">
    ...
    <div class="doc-list">
      <a class="doc-item" href="../documents/1-traiter-les-objections.pdf" target="_blank" rel="noopener">
        <div class="doc-num">1</div>
        <div class="doc-info"><div class="doc-title">Traiter les objections</div><div class="doc-meta">PDF</div></div>
        <div class="doc-open"><svg viewBox="0 0 24 24"><path d="M7 17L17 7"/><polyline points="7 7 17 7 17 17"/></svg></div>
      </a>
      <a class="doc-item" href="../documents/2-200-questions-qui-font-vendre.pdf" target="_blank" rel="noopener">
        <div class="doc-num">2</div>
        <div class="doc-info"><div class="doc-title">200 questions qui font vendre</div><div class="doc-meta">PDF</div></div>
        <div class="doc-open"><svg viewBox="0 0 24 24"><path d="M7 17L17 7"/><polyline points="7 7 17 7 17 17"/></svg></div>
      </a>
      <a class="doc-item" href="../documents/3-neuro-vente.pdf" target="_blank" rel="noopener">
        <div class="doc-num">3</div>
        <div class="doc-info"><div class="doc-title">Neuro-vente</div><div class="doc-meta">PDF</div></div>
        <div class="doc-open"><svg viewBox="0 0 24 24"><path d="M7 17L17 7"/><polyline points="7 7 17 7 17 17"/></svg></div>
      </a>
      <a class="doc-item" href="../documents/4-business-development.pdf" target="_blank" rel="noopener">
        <div class="doc-num">4</div>
        <div class="doc-info"><div class="doc-title">Business Development</div><div class="doc-meta">PDF</div></div>
        <div class="doc-open"><svg viewBox="0 0 24 24"><path d="M7 17L17 7"/><polyline points="7 7 17 7 17 17"/></svg></div>
      </a>
      <a class="doc-item" href="../documents/5-account-executive.pdf" target="_blank" rel="noopener">
        <div class="doc-num">5</div>
        <div class="doc-info"><div class="doc-title">Account Executive</div><div class="doc-meta">PDF</div></div>
        <div class="doc-open"><svg viewBox="0 0 24 24"><path d="M7 17L17 7"/><polyline points="7 7 17 7 17 17"/></svg></div>
      </a>
    </div>
  </div>
...
<div id="p-test" class="page">
  ...
  <div class="test-body">
    ...
    <div id="ts9" style="display:none">
      ...
      <div class="step-nav"><button class="btn bo" onclick="ns(8)">Retour</button><button class="btn bp" onclick="submitADN()">Analyser mon ADN</button></div>
    </div>
    <!-- STEP 10 RESULT -->
    <div id="ts10" style="display:none">
      ...
      <div id="res-block" style="display:none">
        <div class="res-card">
          ...
          <div class="res-d" id="res-denom">sur 100</div>
          <div class="res-t sf" id="res-type">—</div>
          ...
          <div class="res-docs">
            ...
            <div class="res-docs-s">Ajoutez votre CV et votre lettre de motivation pour permettre aux recruteurs de consulter directement votre profil complet.</div>
          ...
          <div class="res-cta">
            <button class="btn blk" style="background:#fff;color:var(--ink);padding:12px;font-family:'Sora',sans-serif;font-weight:800;border-radius:10px;cursor:pointer" onclick="go('swipe')">Plus tard</button>
            <button class="btn blk" style="background:rgba(255,255,255,.1);color:#fff;padding:11px;border:1px solid rgba(255,255,255,.2);border-radius:10px;cursor:pointer;font-family:'Sora',sans-serif;font-weight:700" onclick="go('profile')">Voir mon profil</button>
          </div>
        </div>
      </div>
    </div>
...
<div id="p-profile" class="page">
  <div class="ph">
    <div class="ph-top">
      <div class="ph-av sf" id="prof-av">YS</div>
      <div class="ph-main">
        <div class="ph-name sf" id="prof-name">—</div>
        <div class="ph-title" id="prof-title">—</div>
        <div class="ph-rank" id="prof-rank">—</div>
        <div class="ph-badges">
    ...
    <div class="ph-stats">
      <div><div class="phs-v sf" id="prof-score">—</div><div class="phs-l">ADN Score</div></div>
      <div><div class="phs-v sf" id="prof-matchs">—</div><div class="phs-l">Matchs</div></div>
      <div><div class="phs-v sf" id="prof-status">Actif</div><div class="phs-l">Profil</div></div>
    </div>
  </div>
  <div class="sec">
    <div class="card-sec">
...
async function api(method, path, body) {
  ...
  if (!r.ok) throw new Error(d.message || d.error?.message || d.error || 'Erreur serveur');
  return d;
}
...
function normOffre(o) {
  const co = o.co || o.recruteurs?.entreprise || 'Entreprise';
  const axes = asAxesList(o.axes);
  const score = o.m || Math.min(98, Math.max(60, Math.round((axes[0]?.v || 80) + 5)));
  ...
  return {
    ...
    sz:o.sz || o.recruteurs?.secteur || 'Recruteur SalesForge',
    lieu:o.lieu || 'A definir', type:o.type || 'CDI', g:o.g || 'g1',
    m:score, sal:o.sal || o.salaire || 'Salaire a definir', bn:o.bn || 'Donnee recruteur',
    tags:offerTags, ai:o.ai || 'Offre synchronisee depuis Supabase.',
    axes: axes.length ? axes : [{l:'Fit',v:score},{l:'Drive',v:80},{l:'Closing',v:75}],
    compat:o.compat || {s:score, ax:[{l:'Culture',v:score},{l:'Rythme',v:score},{l:'Ambition',v:score},{l:'Valeurs',v:score}]},
...
function renderHomeCands(c) {
  ...
  document.getElementById('home-cands').innerHTML = c.map(x =>
    `<div class="tr-item"><div class="tr-logo" style="background:${x.bg}">${x.co}</div><div class="tr-info"><div class="tr-title sf">${x.title}</div><div class="tr-sub">${x.sub}</div></div><span class="chip ${cChip(x.statut)}">${x.sl}</span></div>`
  ).join('');
}
...
async function loadOffres() {
  ...
  try {
    OFFRES = (await api('GET', '/offres/deck')).map(normOffre).filter(o => !isOfferLocallySeen(o));
    applyFilter('all');
...
function buildCard(o) {
  ...
  const cc = o.compat.s >= 90 ? 'var(--pu)' : o.compat.s >= 80 ? 'var(--b)' : 'var(--a)';
  d.innerHTML = `
    ...
    <div class="card-top"><div class="card-bg ${o.g}"></div><div class="card-fade"></div>
      ...
      <div class="card-match-box"><div class="card-match-n sf">${o.m}</div><div class="card-match-l">Match</div></div>
      <div class="card-co"><div class="card-co-n sf">${o.co}</div><div class="card-co-s">${o.sz}</div></div>
    </div>
    <div class="card-body">
      <div class="card-title sf">${o.titre}</div>
      <div class="card-meta"><span class="cmi"><svg viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>${o.lieu}</span><span class="cmi"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/></svg>${o.dur}</span></div>
      <div class="card-salary"><div class="card-sal-v sf">${o.sal}</div><div class="card-bench-note">${o.bn}</div></div>
      <div class="compat-bar"><div class="compat-row"><span class="compat-lbl sf">Compatibilité culturelle</span><span class="compat-score sf" style="color:${cc}">${o.compat.s}%</span></div><div class="compat-axes">${o.compat.ax.map(a=>`<div class="cx"><div class="cx-l">${a.l}</div><div class="cx-bar"><div class="cx-fill" style="width:${a.v}%;background:${cc}"></div></div><div class="cx-v" style="color:${cc}">${a.v}</div></div>`).join('')}</div></div>
      <div class="card-ai"><div style="margin-bottom:6px"><span class="chip chip-b"><span class="chip-dot"></span>Analyse IA</span></div><div class="card-ai-text">${o.ai}</div><div class="card-ai-axes">${o.axes.map(a=>`<div class="max"><div class="max-l">${a.l}</div><div class="max-bar"><div class="max-fill" style="width:${a.v}%"></div></div><div class="max-v sf">${a.v}</div></div>`).join('')}</div></div>
    </div>`;
...
function attachDrag(card, offre) {
  ...
  const s = e => { if (card !== top()) return; if (e.cancelable) e.preventDefault(); on = true; const p = point(e); sx=p.clientX; sy=p.clientY; cx=0; cy=0; card.style.transition='none'; };
  const m = e => {
    if (!on) return;
...
async function doSwipe(action, offre) {
  try {
...
function ns(n) {
  ...
  document.getElementById('test-sl').textContent = 'Étape '+n+' sur 10';
  document.getElementById('test-st').textContent = STEPS[n];
  document.getElementById('test-pf').style.width = (n*10) + '%';
  if (n===10) runAnalysis();
  window.scrollTo(0,0);
}
...
function renderResult(r) {
  ...
  document.getElementById('res-score').textContent=r.score; document.getElementById('res-denom').textContent=r.rank;
  document.getElementById('res-type').textContent=r.type; document.getElementById('res-desc').textContent=r.desc;
  document.getElementById('res-axes').innerHTML=r.axes.map(a=>`<div class="res-ax"><div class="res-ax-v sf">${a.v}</div><div class="res-ax-l">${a.l}</div></div>`).join('');
  document.getElementById('res-tags').innerHTML=r.tags.map(t=>`<span class="res-tp">${t}</span>`).join('');
...
function setMessageBadges(unreadCount) {
  ...
  document.title = hasUnread ? 'Nouveaux messages - SalesForge' : 'Espace candidat - SalesForge';
}
...
async function loadMessages() {
  ...
  try {
    ...
    document.getElementById('msgs-list').innerHTML = THREADS.length ? THREADS.map(t=>
      `<div class="msg-th${t.ur?' ur':''}" onclick="openThread('${t.id}')"><div class="msg-av" style="background:${esc(t.bg)}">${esc(t.av)}</div><div class="msg-b"><div class="msg-top"><span class="msg-n sf">${esc(t.nom)}</span><span class="msg-time">${esc(t.time)}</span></div><div class="msg-prev">${esc(t.prev)}</div>${t.status?`<div class="msg-foot"><span class="msg-status${t.ur?' unread':''}">${esc(messageStatusLabel(t.status))}</span></div>`:''}</div>${t.ur?'<div class="ur-dot"></div>':''}</div>`
    ).join('') : '<div class="loading-state">Aucune conversation pour le moment</div>';
...
async function openThread(id, surface = 'messages') {
  ...
  document.getElementById('chat-sub').textContent = thread.prev || 'Conversation SalesForge';
...
async function loadProfile() {
  if (!USER) return;
  document.getElementById('prof-name').textContent=(USER.prenom||'')+' '+(USER.nom||'');
  try {
...
function renderProfile(p, a) {
  p.type_profil = p.type_profil || p.axes?.resultat?.type || p.axes?.resultat?.type_profil;
  p.rank = p.rank || p.axes?.resultat?.rank;
  const avatarLabel = ((p.prenom||'?')[0]+(p.nom||'?')[0]).toUpperCase();
  document.getElementById('prof-name').textContent=(p.prenom||'')+' '+(p.nom||'');
  renderProfileAvatar(p, avatarLabel);
  document.getElementById('prof-title').textContent=p.type_profil||'—'; document.getElementById('prof-rank').textContent=p.rank||'—';
...
async function saveProfileEdit() {
  ...
  try {
    const p = await api('PUT', '/candidats/profil', { prenom, nom });
    USER = { ...(USER || {}), prenom: p.prenom, nom: p.nom };
    localStorage.setItem('sf_user', JSON.stringify(USER));
    document.getElementById('prof-name').textContent=(p.prenom||'')+' '+(p.nom||'');
    renderProfileAvatar({ ...USER, avatar_url: PROFILE_AVATAR_URL }, ((p.prenom||'?')[0]+(p.nom||'?')[0]).toUpperCase());
...
function renderProfileCV(p) {
  ...
  badge.classList.toggle('on', Boolean(name || PROFILE_CV_URL));
  ...
  if (deleteBtn) deleteBtn.disabled = !Boolean(name || PROFILE_CV_URL);
}
...
function renderMotivationFile(p) {
  ...
  badge.classList.toggle('on', Boolean(name || PROFILE_MOTIVATION_URL));
  ...
  if (deleteBtn) deleteBtn.disabled = !Boolean(name || PROFILE_MOTIVATION_URL);
}
...
async function updateAvatar(input) {
  ...
  try {
    ...
    const label = ((USER?.prenom||'?')[0]+(USER?.nom||'?')[0]).toUpperCase();
    renderProfileAvatar({ avatar_url: PROFILE_AVATAR_URL }, label);
...
const PLANS = {
  month:[
    ...
    {n:'Premium',f:'Commercial actif',p:19,u:'/mois',h:'Sans engagement',feats:['Swipes et candidatures illimités','10 contacts recruteurs/mois','ADN détaillé 8 axes','Lettre de motivation écrite','Benchmark salarial précis','Préparation entretien'],cta:'Essai 14j gratuit',pop:true},
    {n:'Gold',f:'Top performer',p:39,u:'/mois',h:'Sans engagement',feats:['Tout Premium inclus','Contacts illimités','Mise en avant ×3','5 Super-likes/mois','Ressources avancées','Certification SalesForge','Offres exclusives','Communauté Closers Elite'],cta:'Passer Gold'},
  ...
  year:[
    ...
    {n:'Gold',f:'Top performer',p:31,old:39,u:'/mois',h:'Facturé 372 €/an · -96 €',feats:['Tout Premium inclus','Contacts illimités','Mise en avant ×3','5 Super-likes/mois','Ressources avancées','Certification SalesForge','Offres exclusives','Communauté Closers Elite'],cta:'Économiser 20%'},
...
function renderPlans() {
  document.getElementById('plans-row').innerHTML=PLANS[BILLING].map(p=>`
    <div class="plan-card${p.pop?' pop':''}${p.dk?' dk':''}">
      ${p.pop?'<div class="pop-ribbon">Le plus populaire</div>':''}
      ...
      <button class="pc-btn" onclick="selectPlan('${p.n.toLowerCase()}','${BILLING}')">${p.cta}</button>
    </div>`).join('');
}
