...
<div id="p-dashboard" class="page">
  ...
  <div class="sec dash-actions">
    ...
    <button class="btn bo blk" style="margin-bottom:8px" onclick="go('offres')">Toutes mes offres</button>
...
<div id="p-settings" class="page">
  <div class="sec">
    ...
    <div style="background:var(--w);border:1px solid var(--bd);border-radius:12px;overflow:hidden">
      <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-bottom:1px solid var(--bd);cursor:pointer" onclick="go('offres')"><div style="width:31px;height:31px;border-radius:8px;background:var(--bs);color:var(--b);display:flex;align-items:center;justify-content:center"><svg width="15" height="15" viewBox="0 0 24 24" style="stroke:currentColor;fill:none;stroke-width:2"><rect x="4" y="4" width="16" height="16" rx="2"/><line x1="8" y1="9" x2="16" y2="9"/><line x1="8" y1="14" x2="14" y2="14"/></svg></div><div style="flex:1;font-size:13px;font-weight:600">Mes offres</div></div>
...
async function api(method, path, body) {
  ...
  if (!r.ok) {
    ...
    if (d.error === 'OFFER_LIMIT_REACHED') {
      toast('Limite de 3 offres atteinte — passez au plan Enterprise');
      go('pricing');
...
function saveLastPage(p) { if (p !== 'auth' && p !== 'pricing') localStorage.setItem(lastPageKey(), p); }
...
// ------------------------------------------------------------
function goAppLogin() {
  window.top.location.href = 'salesforge_app.html?role=recruteur';
}
function goLanding() {
...
async function loadPipeline() {
  ...
  try {
    PIPELINE = await api('GET', '/recruteurs/pipeline');
    renderPipeline();
  } catch(e) { document.getElementById('pipe-cols').innerHTML = '<div class="loading-state">Erreur</div>'; }
}
  ...
  document.getElementById('pipe-cols').innerHTML = PIPE_STAGES.map(stage => `
    <div class="pipe-col">
      ...
      ${(PIPELINE[stage.key]||[]).map(c => `
        <div class="pipe-card" draggable="false" ondragstart="return false">
  ...
  if (!offers.length) {
    list.innerHTML = `
      <div class="loading-state">
        Aucune offre publiée pour le moment.<br><br>
  ...
  try {
    ...
    document.getElementById('msgs-list').innerHTML = THREADS.length ? THREADS.map(t => `
      <div class="msg-th${t.ur?' ur':''}" onclick="openThread('${t.id}')">
        ...
        ${t.ur?'<div class="ur-dot"></div>':''}
      </div>`).join('') : '<div class="loading-state">Aucune conversation pour le moment</div>';
...
async function selectPlan(plan, billing) {
  try {
    const mapped = {business:'starter', enterprise:'enterprise', partenaire:'pro'}[plan] || plan;
...
function renderPlans() {
  document.getElementById('plans-row').innerHTML = PLANS[BILLING].map(p => `
    <div class="plan-card${p.pop?' pop':''}${p.dk?' dk':''}">
      ${p.pop?'<div class="pop-ribbon">Le plus choisi</div>':''}
...
if (hydrateRecruiterSession()) {
  ...
  api('GET', '/recruteurs/stats?t=' + Date.now())
    .then(() => { HAS_PLAN = true; })
    .catch(() => { HAS_PLAN = false; })
    .finally(() => {
      go(start ? existingPageOrDashboard(start) : getLastPage());
    });
} else {
  goAppLogin();
}
